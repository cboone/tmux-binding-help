...
COLOR_CMD=$'\033[0;37m'            # White for commands
...
render() {
    ...
    # ── Header ──
    printf '\033[K%s tmux help%s  %s%d bindings%s\n' \
...
move_down() { local max=$(( ${#VISIBLE[@]} - 1 )); (( SELECTED < max )) && SELECTED=$(( SELECTED + 1 )) || true; }
move_top()  { SELECTED=0; SCROLL_OFFSET=0; }
    ...
    if [[ "$key" == $'\x1b' ]]; then
        ...
        if [[ "$seq" == "[" ]]; then
            ...
            case "$seq" in
                ...
                H) printf 'HOME' ;;
                F) printf 'END' ;;
                5) IFS= read -rsn1 _ 2>/dev/null; printf 'PGUP' ;;
                6) IFS= read -rsn1 _ 2>/dev/null; printf 'PGDN' ;;
                *) printf 'UNKNOWN' ;;
            esac
        else
            printf 'UNKNOWN'
